RK_get_perf <- function(lp, Age, status, group, fh, psa, ref = NULL, swc.switch = TRUE, swc.caco = NULL){
  # Returns: 
  ...
  hareF = factor(group, levels=c('EUR','AFR','HIS','OTHER','NATIVE', 'ASN', 'UNKNOWN', 'PACIFIC'),ordered=FALSE)
  tmp.df = data.frame(Age = Age, status = status, lp = lp, group=hareF, fh=fh, psa=psa, wvec = swc.wvec)
  if (swc.switch == TRUE){
    cxph = coxph(Surv(Age, status) ~ lp + group + fh + psa, data = tmp.df, weights = swc.wvec)
    
  } else{
    cxph = coxph(Surv(Age, status) ~ lp + group + fh + psa, data = tmp.df)
    print(summary(cxph))
  }
  ...
  #get coeficients of race
  hare_list = list(beta_afr = as.numeric(cxph$coefficients[2]), p_afr = as.numeric(summary(cxph)$coefficients[,5][2]), beta_his = as.numeric(cxph$coefficients[3]), p_his = as.numeric(summary(cxph)$coefficients[,5][3]), beta_oth = as.numeric(cxph$coefficients[4]), p_oth = as.numeric(summary(cxph)$coefficients[,5][4]),beta_native = as.numeric(cxph$coefficients[5]), p_native = as.numeric(summary(cxph)$coefficients[,5][5]),beta_asn = as.numeric(cxph$coefficients[6]), p_asn = as.numeric(summary(cxph)$coefficients[,5][6]),beta_unknown = as.numeric(cxph$coefficients[7]), p_unknown = as.numeric(summary(cxph)$coefficients[,5][7]), beta_pac = as.numeric(cxph$coefficients[8]), p_pac = as.numeric(summary(cxph)$coefficients[,5][8]))  
  ...
  psa_list = list(beta_psa = as.numeric(cxph$coefficients[10]), p_psa = as.numeric(summary(cxph)$coefficients[,5][10]))
  perf_list = c(perf_list, psa_list)
