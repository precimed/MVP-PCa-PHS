...
# script to run medicare univariable for race/ethnicity groups
...
output=/scratch/scratch10/mvp022/PHS290/data/univariable/race_eth_medicare_annot
boot_input=/scratch/scratch10/mvp022/PHS290/data/bootstraps/eth/medicare_annot
release=release4
...
Rscript a1_test_get_perf_medicare.R $input/$group.male.prs.$release.tsv $var $output/single/$group.$release.$var.txt
...
$BSUB_CMD Rscript a1_test_get_perf_medicare.R $boot_input/$group.$i.$var.tsv $var $output/single/$group.$release.$i.$var.txt
done
