RK_get_perf <- function(lp, Age, status, group, fh, ref = NULL, swc.switch = TRUE, swc.caco = NULL){
  ...
  groupF = factor(group, levels=c('EUR','AFR','AMR','CSA','MID','EAS'),ordered=FALSE)
  ...
  group_list = list(beta_afr = as.numeric(cxph$coefficients[2]), p_afr = as.numeric(summary(cxph)$coefficients[,5][2]), beta_amr = as.numeric(cxph$coefficients[3]), p_amr = as.numeric(summary(cxph)$coefficients[,5][3]), beta_csa = as.numeric(cxph$coefficients[4]), p_csa = as.numeric(summary(cxph)$coefficients[,5][4]),beta_mid = as.numeric(cxph$coefficients[5]), p_mid = as.numeric(summary(cxph)$coefficients[,5][5]),beta_eas = as.numeric(cxph$coefficients[6]), p_eas = as.numeric(summary(cxph)$coefficients[,5][6]))
