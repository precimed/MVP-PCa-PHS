RK_get_perf <- function(lp, Age, status, group, fh, ref = NULL, swc.switch = TRUE, swc.caco = NULL){
  # Returns: 
  ...
  print(hareF)
  tmp.df = data.frame(Age = Age, status = status, lp = lp, group=groupF, fh=fh,  wvec = swc.wvec)
  if (swc.switch == TRUE){
    cxph = coxph(Surv(Age, status) ~ lp + group + fh, data = tmp.df, weights = swc.wvec)
    
  } else{
    cxph = coxph(Surv(Age, status) ~ lp + group + fh, data = tmp.df)
    print(summary(cxph))
  }
  ...
  group_list = list(beta_afr = as.numeric(cxph$coefficients[2]), p_afr = as.numeric(summary(cxph)$coefficients[,5][2]), beta_his = as.numeric(cxph$coefficients[3]), p_his = as.numeric(summary(cxph)$coefficients[,5][3]), beta_oth = as.numeric(cxph$coefficients[4]), p_oth = as.numeric(summary(cxph)$coefficients[,5][4]),beta_native = as.numeric(cxph$coefficients[5]), p_native = as.numeric(summary(cxph)$coefficients[,5][5]),beta_asn = as.numeric(cxph$coefficients[6]), p_asn = as.numeric(summary(cxph)$coefficients[,5][6]),beta_unknown = as.numeric(cxph$coefficients[7]), p_unknown = as.numeric(summary(cxph)$coefficients[,5][7]), beta_oth = as.numeric(cxph$coefficients[8]), p_oth = as.numeric(summary(cxph)$coefficients[,5][8]))
