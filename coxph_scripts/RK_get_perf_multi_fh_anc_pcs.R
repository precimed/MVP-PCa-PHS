RK_get_perf <- function(lp, Age, status, group, fh, pc1, pc2, pc3, pc4, pc5, pc6, pc7, pc8, pc9, pc10,ref = NULL, swc.switch = TRUE, swc.caco = NULL){
  ...
  # Returns: 
  # list with following metrics: beta values, p values, swc, HR98_50, HR20_50, HR80_20 
  ...
  hareF = factor(group, levels=c('EUR','AFR','AMR','CSA','MID','EAS'),ordered=FALSE)
  tmp.df = data.frame(Age = Age, status = status, lp = lp, group=hareF, fh=fh, pc1=pc1, pc2=pc2, pc3=pc3, pc4=pc4, pc5=pc5, pc6=pc6, pc7=pc7, pc8=pc8, pc9=pc9, pc10=pc10, wvec = swc.wvec)
  if (swc.switch == TRUE){
  ...
  group_list = list(beta_afr = as.numeric(cxph$coefficients[2]), p_afr = as.numeric(summary(cxph)$coefficients[,5][2]), beta_amr = as.numeric(cxph$coefficients[3]), p_amr = as.numeric(summary(cxph)$coefficients[,5][3]), beta_csa = as.numeric(cxph$coefficients[4]), p_csa = as.numeric(summary(cxph)$coefficients[,5][4]),beta_mid = as.numeric(cxph$coefficients[5]), p_mid = as.numeric(summary(cxph)$coefficients[,5][5]),beta_csa = as.numeric(cxph$coefficients[6]), p_eas = as.numeric(summary(cxph)$coefficients[,5][6]))
  perf_list = c(perf_list, group_list)  

  #get coefficients of FH
  fh_list = list(beta_fh = as.numeric(cxph$coefficients[7]), p_fh = as.numeric(summary(cxph)$coefficients[,5][7]))
  perf_list = c(perf_list, fh_list)

  #get coefficients of PC1-10
  pc_list = list(beta_pc1 = as.numeric(cxph$coefficients[8]), p_pc1 = as.numeric(summary(cxph)$coefficients[,5][8]), beta_pc2 = as.numeric(cxph$coefficients[9]), p_pc2 = as.numeric(summary(cxph)$coefficients[,5][9]),beta_pc3 = as.numeric(cxph$coefficients[10]), p_pc3 = as.numeric(summary(cxph)$coefficients[,5][10]), beta_pc4 = as.numeric(cxph$coefficients[11]), p_pc4 = as.numeric(summary(cxph)$coefficients[,5][11]), beta_pc5 = as.numeric(cxph$coefficients[12]), p_pc5 = as.numeric(summary(cxph)$coefficients[,5][12]), beta_pc6 = as.numeric(cxph$coefficients[13]), p_pc6 = as.numeric(summary(cxph)$coefficients[,5][13]), beta_pc7 = as.numeric(cxph$coefficients[14]), p_pc7 = as.numeric(summary(cxph)$coefficients[,5][14]), beta_pc8 = as.numeric(cxph$coefficients[15]), p_pc8 = as.numeric(summary(cxph)$coefficients[,5][15]), beta_pc9 = as.numeric(cxph$coefficients[16]), p_pc9 = as.numeric(summary(cxph)$coefficients[,5][16]),beta_pc10 = as.numeric(cxph$coefficients[17]), p_pc10 = as.numeric(summary(cxph)$coefficients[,5][17]))
