...
# script to run psa univariable with race/ethnicity groups
...
for type in psa_test psa_test_yr psa_first_year
do
input=/group/research/mvp022/PHS/data/phs_files/phs290_eth_psa
output=/scratch/scratch10/mvp022/PHS290/data/univariable/ethnicity/$type
boot_input=/scratch/scratch10/mvp022/PHS290/data/bootstraps/eth/psa
release=release4

# run with full dataset
mkdir -p  $output/single/
Rscript a1_test_get_perf_psa.R $input/$group.male.prs.psa.$release.tsv $var $type $output/single/$group.$release.$var.txt
...
# run with bootstraps
	...
	$BSUB_CMD Rscript a1_test_get_perf_psa.R $boot_input/$group.$i.$var.tsv $var $type $output/single/$group.$release.$i.$var.txt
	done
