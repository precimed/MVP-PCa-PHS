...
#BSUB -q medium
#BSUB -M 2500
#BSUB -J bootstrap[1-27]%3
#BSUB -o ./out/%J.%I.stdout
#BSUB -e ./err/%J.%I.stderr

date

# boostrapping script for non-medicare patients
...
type=medicare
input=/group/research/mvp022/PHS/data/phs_files/phs290_eth_medicare
output=/scratch/scratch10/mvp022/PHS290/data/bootstraps/eth/$type/$group

mkdir -p $output
	
sed "s|INSERTFILE|${input}/${group}.male.prs.medicare.release4.tsv|g" bootstrap.sh > bootstrap.$group.$type.$var.sh
